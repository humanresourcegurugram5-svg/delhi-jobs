...
FONT_URLS = {
    "Bold.ttf":    "https://github.com/google/fonts/raw/main/ofl/montserrat/static/Montserrat-Bold.ttf",
    "Black.ttf":   "https://github.com/google/fonts/raw/main/ofl/montserrat/static/Montserrat-Black.ttf",
    "Regular.ttf": "https://github.com/google/fonts/raw/main/ofl/montserrat/static/Montserrat-Regular.ttf",
    "Light.ttf":   "https://github.com/google/fonts/raw/main/ofl/montserrat/static/Montserrat-Light.ttf",
    "Serif.ttf":   "https://github.com/google/fonts/raw/main/ofl/playfairdisplay/static/PlayfairDisplay-Bold.ttf",
    "SerifReg.ttf":"https://github.com/google/fonts/raw/main/ofl/playfairdisplay/static/PlayfairDisplay-Regular.ttf",
    "Mono.ttf":    "https://github.com/google/fonts/raw/main/ofl/spacemono/SpaceMono-Regular.ttf",
    "MonoBold.ttf":"https://github.com/google/fonts/raw/main/ofl/spacemono/SpaceMono-Bold.ttf",
...
def smart_salary(salary, title, company, experience):
    garbage = ["not disclosed","na","not mentioned","best in industry",
               "not specified","-","negotiable","as per industry","","none"]
    raw = str(salary).strip().lower() if salary else ""
    has_real = raw and raw not in garbage and len(raw) > 3

    if has_real:
        return str(salary).strip(), "real"

    tier  = get_company_tier(company)
    title_lower = title.lower()
    exp_b = get_exp_bracket(experience)

    # Tier 1 — premium label
    if tier == "tier1":
        return "Best in Industry", "premium"

    # Try salary map estimate
    for key, ranges in SALARY_MAP.items():
        if key in title_lower:
            return ranges.get(exp_b, "Competitive Salary"), "estimated"

    # Startup — hike label
    if tier == "startup":
        return random.choice(HIKE_OPTIONS), "hike"

    # Tier 2 known company — market rate
    if tier == "tier2":
        return "Competitive Salary", "market"
...
def salary_label(salary_type):
    labels = {
        "real":      "SALARY",
        "estimated": "EST. SALARY",
        "premium":   "COMPENSATION",
        "hike":      "SALARY HIKE",
        "market":    "MARKET RATE",
        "fallback":  "SALARY",
    }
    return labels.get(salary_type, "SALARY")
...
def load_fonts():
    ...
    return {
        "black_xl":  f("Black.ttf",   130),
        "black_lg":  f("Black.ttf",   100),
        "black_md":  f("Black.ttf",   76),
        "black_sm":  f("Black.ttf",   56),
        "bold_xl":   f("Bold.ttf",    100),
        "bold_lg":   f("Bold.ttf",    76),
        "bold_md":   f("Bold.ttf",    56),
        "bold_sm":   f("Bold.ttf",    38),
        "bold_xs":   f("Bold.ttf",    28),
        "bold_xxs":  f("Bold.ttf",    22),
        "reg_md":    f("Regular.ttf", 38),
        "reg_sm":    f("Regular.ttf", 28),
        "reg_xs":    f("Regular.ttf", 22),
        "light_md":  f("Light.ttf",   38),
        "light_sm":  f("Light.ttf",   28),
        "serif_xl":  f("Serif.ttf",   120),
        "serif_lg":  f("Serif.ttf",   96),
        "serif_md":  f("Serif.ttf",   72),
        "serif_sm":  f("SerifReg.ttf",50),
        "mono_md":   f("Mono.ttf",    34),
        "mono_sm":   f("Mono.ttf",    26),
        "mono_xs":   f("Mono.ttf",    20),
        "mono_bold": f("MonoBold.ttf",34),
        "noto_md":   f("Noto.ttf",    38),
...
# ════════════════════════════════════════════════════════════════════════════
# STYLE FUNCTIONS  — each returns a saved image path
# ════════════════════════════════════════════════════════════════════════════

def style_warm_cream(job, fonts, output):
    img  = Image.new("RGB", (WIDTH, HEIGHT), "#fdf6ee")
    draw = ImageDraw.Draw(img)
    # decorative circle
    draw.ellipse([WIDTH-420, HEIGHT-420, WIDTH+160, HEIGHT+160],
                 outline="#e8ddd0", width=60)
    # top labels
    draw.text((72, 80), "DELHI / GURUGRAM", font=fonts["bold_xxs"],
              fill="#b49060")
    draw_rounded_rect(draw, [WIDTH-240, 65, WIDTH-50, 125], 6,
                      outline="#2c1f0e", width=3)
    draw.text((WIDTH-220, 95), "HIRING NOW", font=fonts["bold_xxs"],
              fill="#2c1f0e")
    # title
    lines = wrap_title(job["title"], 18)
    y = 200
    for line in lines:
        draw.text((72, y), line, font=fonts["serif_lg"], fill="#2c1f0e")
        y += 110
    draw.text((72, y+10), truncate(job["company"], 32),
              font=fonts["bold_sm"], fill="#6b4f2e")
    # divider
    draw.line([(72, 820), (WIDTH-72, 820)], fill="#e8ddd0", width=3)
    # salary row
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    draw.text((72, 848),  slabel,         font=fonts["bold_xxs"], fill="#b49060")
    draw.text((72, 878),  sal,            font=fonts["noto_md"],  fill="#2c1f0e")
    draw.text((500, 848), "EXPERIENCE",   font=fonts["bold_xxs"], fill="#b49060")
    draw.text((500, 878), truncate(job.get("experience","Open to all"), 14),
              font=fonts["bold_xs"], fill="#2c1f0e")
    draw.text((840, 848), "APPLY",        font=fonts["bold_xxs"], fill="#b49060")
    draw.text((840, 878), "Link in bio",  font=fonts["bold_xxs"], fill="#2c1f0e")
    # hashtags
    draw.text((72, 970), "#DelhiJobs  #GurugramJobs  #JobAlert",
              font=fonts["reg_xs"], fill="#c4a882")
    draw.text((72, 1010), "@delhi_gurugram_jobs",
              font=fonts["reg_xs"], fill="#e8ddd0")
...
def style_dark_bold(job, fonts, output):
    ...
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), "@ " + truncate(job["company"], 28),
              font=fonts["bold_md"], fill="#ff6b35")
    draw.text((72, y+80), truncate(job.get("location","Delhi / Gurugram"), 30),
              font=fonts["reg_sm"], fill="#888888")
    # bottom info box
    ...
    draw.text((560, 875), truncate(job.get("experience","Open"), 12),
              font=fonts["bold_sm"], fill="#ffffff")
    draw.line([(820, 840), (820, 955)], fill="#333333", width=3)
    draw_rounded_rect(draw, [840, 855, WIDTH-80, 945], 10, fill="#ff6b35")
    draw.text((880, 900), "APPLY", font=fonts["bold_xs"],
              fill="#ffffff", anchor="lm")
    draw.text((72, 985), "@delhi_gurugram_jobs",
              font=fonts["mono_xs"], fill="#333333")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_split_layout(job, fonts, output):
    ...
    draw.text((130, 600), company_short, font=fonts["bold_md"],
              fill="#7eb9ff", anchor="mm")
    # right content
    cat = job.get("category","IT").upper()
    draw.text((300, 72), cat + "  |  GURUGRAM",
              font=fonts["bold_xxs"], fill="#999999")
    lines = wrap_title(job["title"], 16)
    y = 160
    for line in lines:
    ...
    draw.text((300, y+50), truncate(job["company"], 30),
              font=fonts["bold_sm"], fill="#1a3c5e")
    # skills tags
    ...
    tx, ty = 300, y + 140
    for sk in skills[:4]:
        draw_rounded_rect(draw, [tx, ty, tx+len(sk)*16+20, ty+46], 6,
                          fill="#f0f4ff", outline="#1a3c5e", width=2)
        draw.text((tx+10, ty+23), sk, font=fonts["bold_xxs"],
                  fill="#1a3c5e", anchor="lm")
        tx += len(sk)*16+36
    ...
    draw.text((900, 950), "Apply", font=fonts["bold_xs"],
...
def style_minimal_line(job, fonts, output):
    ...
    draw = ImageDraw.Draw(img)
    draw.text((72, 72),  "NO. {:02d}".format(random.randint(1,99)),
              font=fonts["mono_xs"], fill="#999999")
    draw.text((WIDTH-72, 72), today_str(), font=fonts["mono_xs"],
              fill="#999999", anchor="ra")
    draw.line([(72, 130), (WIDTH-72, 130)], fill="#e0e0e0", width=2)
    cat = job.get("category","GENERAL").upper()
    draw.text((72, 160), cat, font=fonts["bold_xxs"], fill="#999999")
    lines = wrap_title(job["title"], 18)
    y = 220
    for line in lines:
    ...
    draw.text((72, y+85),   truncate(job["company"], 20),
              font=fonts["bold_sm"], fill="#333333")
    draw.text((400, y+85),  truncate(job.get("location","Delhi NCR"), 16),
              font=fonts["bold_sm"], fill="#333333")
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((400, y+205), truncate(job.get("experience","Open"), 14),
              font=fonts["bold_sm"], fill="#333333")
    draw.line([(72, HEIGHT-120), (WIDTH-72, HEIGHT-120)], fill="#e0e0e0", width=2)
    draw.text((72, HEIGHT-88),     "@delhi_gurugram_jobs",
              font=fonts["mono_xs"], fill="#999999")
    draw.text((WIDTH-72, HEIGHT-88), "LINK IN BIO",
              font=fonts["mono_xs"], fill="#111111", anchor="ra")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_color_block(job, fonts, output):
    ...
    draw.text((206, 101), "HR & ADMIN", font=fonts["bold_xxs"],
              fill="#ddd6fe", anchor="mm")
    lines = wrap_title(job["title"], 16)
    y = 200
    for line in lines:
        draw.text((72, y), line, font=fonts["black_xl"], fill="#ffffff")
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#a78bfa")
    # frosted panel
    draw_rounded_rect(draw, [72, 820, WIDTH-72, 960], 20, fill="#ffffff1a",
                      outline="#ffffff26", width=2)
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((640, 878), truncate(job.get("experience","Open"),12),
              font=fonts["bold_sm"], fill="#ffffff")
    draw_rounded_rect(draw, [72, 975, WIDTH-72, 1050], 14, fill="#7c3aed")
    draw.text((WIDTH//2, 1012), "APPLY NOW  -  LINK IN BIO",
              font=fonts["bold_xs"], fill="#ffffff", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_midnight_gold(job, fonts, output):
    ...
    draw.text((WIDTH//2, 110), "* PREMIUM OPENING *",
              font=fonts["bold_xxs"], fill="#c9a84c", anchor="mm")
    lines = wrap_title(job["title"], 18)
    y = 240
    for line in lines:
        draw.text((WIDTH//2, y), line, font=fonts["serif_lg"], fill="#ffffff",
                  anchor="mm")
        y += 130
    draw.text((WIDTH//2, y+30), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#c9a84c", anchor="mm")
    draw.line([(WIDTH//2-100, y+100), (WIDTH//2+100, y+100)],
              fill="#c9a84c44", width=2)
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((700, y+220), truncate(job.get("experience","Open"),12),
              font=fonts["bold_sm"], fill="#ffffff", anchor="mm")
    draw.text((WIDTH//2, HEIGHT-90), "APPLY  -  LINK IN BIO",
              font=fonts["bold_xxs"], fill="#666666", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_blueprint(job, fonts, output):
    ...
    for x in range(0, WIDTH, 80):
        draw.line([(x,0),(x,HEIGHT)], fill="#ffffff08", width=1)
    for y in range(0, HEIGHT, 80):
        draw.line([(0,y),(WIDTH,y)], fill="#ffffff08", width=1)
    draw_rounded_rect(draw, [40, 40, WIDTH-40, HEIGHT-40], 8,
                      outline="#64a0ff33", width=2)
    draw.text((72, 80), "SYS.JOB_ID: 2026-{:04d}".format(random.randint(1,9999)),
              font=fonts["mono_xs"], fill="#5b9bd5")
    lines = wrap_title(job["title"], 16)
    y = 180
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["mono_md"], fill="#5b9bd5")
    draw.text((72, y+70), truncate(job.get("location","Delhi NCR"), 24),
              font=fonts["mono_xs"], fill="#3a6a9a")
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    draw_rounded_rect(draw, [72, y+140, WIDTH-72, y+280], 8,
                      outline="#64a0ff33", width=2)
    ...
    draw.text((600, y+205), truncate(job.get("experience","Open"),14),
              font=fonts["mono_bold"], fill="#e8f4ff")
    draw.text((72, HEIGHT-80), "-> APPLY  |  LINK IN BIO",
              font=fonts["mono_sm"], fill="#5b9bd5")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_typewriter(job, fonts, output):
    ...
    draw = ImageDraw.Draw(img)
    draw.text((72, 72), "--- job_post_{:03d}.txt ---".format(random.randint(1,999)),
              font=fonts["mono_xs"], fill="#999999")
    ...
    draw.text((72, y+60), truncate(job["company"], 28),
              font=fonts["mono_bold"], fill="#111111")
    draw.text((72, y+130), "LOCATION:", font=fonts["mono_sm"], fill="#666666")
    draw.text((72, y+180), truncate(job.get("location","Delhi NCR"), 24),
              font=fonts["mono_sm"], fill="#111111")
    ...
    draw.text((580, y+320), truncate(job.get("experience","Open"),14),
              font=fonts["mono_bold"], fill="#111111")
...
def style_neon_night(job, fonts, output):
    ...
    draw.text((72, 80), "> LIVE  |  HIRING NOW",
              font=fonts["mono_sm"], fill="#00ff88")
    lines = wrap_title(job["title"], 16)
    y = 180
    for line in lines:
        draw.text((72, y), line, font=fonts["black_xl"], fill="#ffffff")
        y += 140
    draw.text((72, y+10), "@ " + truncate(job["company"], 26),
              font=fonts["bold_sm"], fill="#00ff88")
    draw.text((72, y+80), truncate(job.get("location","Delhi NCR"), 28),
              font=fonts["mono_xs"], fill="#444444")
    draw_rounded_rect(draw, [72, y+140, WIDTH-72, y+300], 16,
                      fill="#ffffff0a", outline="#00ff8833", width=2)
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((620, y+200), truncate(job.get("experience","Open"),12),
              font=fonts["bold_sm"], fill="#ffffff")
    draw_rounded_rect(draw, [72, y+320, WIDTH-72, y+420], 14, fill="#00ff88")
    draw.text((WIDTH//2, y+370), "APPLY  -  LINK IN BIO",
              font=fonts["bold_sm"], fill="#000000", anchor="mm")
    draw.text((72, HEIGHT-60), "@delhi_gurugram_jobs",
              font=fonts["mono_xs"], fill="#333333")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_red_alert(job, fonts, output):
    ...
    draw.text((72, 80), "URGENT REQUIREMENT",
              font=fonts["bold_xxs"], fill="#ffffff99")
    lines = wrap_title(job["title"], 16)
    y = 150
    for line in lines:
        draw.text((72, y), line, font=fonts["black_xl"], fill="#ffffff")
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#ffffff99")
    # white body
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((760, 575), truncate(job.get("experience","Open"),12),
              font=fonts["bold_sm"], fill="#1a0000", anchor="mm")
    draw_rounded_rect(draw, [72, 680, WIDTH-72, 800], 14, fill="#dc2626")
    draw.text((WIDTH//2, 740), "APPLY NOW  -  LINK IN BIO",
              font=fonts["bold_sm"], fill="#ffffff", anchor="mm")
    draw.text((72, 840), truncate(job.get("location","Delhi NCR"), 28),
              font=fonts["reg_sm"], fill="#999999")
    draw.text((72, 890), "@delhi_gurugram_jobs",
              font=fonts["reg_xs"], fill="#cccccc")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_teal_modern(job, fonts, output):
    ...
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#0d9488")
    draw.text((72, y+80), truncate(job.get("location","Delhi NCR"), 28),
              font=fonts["reg_sm"], fill="#5e9e96")
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    draw_rounded_rect(draw, [72, y+150, WIDTH-72, y+310], 16,
                      fill="#ffffff", outline="#b2e8df", width=3)
    ...
    draw.text((620, y+210), truncate(job.get("experience","Open"),14),
              font=fonts["bold_sm"], fill="#0f2825")
    draw.text((WIDTH//2, HEIGHT-80), "APPLY  |  LINK IN BIO",
              font=fonts["bold_xs"], fill="#0d9488", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_saffron(job, fonts, output):
    ...
    draw.text((WIDTH-160, 97), "HIRING", font=fonts["bold_sm"],
              fill="#ffffff", anchor="mm")
    draw.text((72, 80), truncate(job.get("location","Gurugram"), 22).upper() + "  |  FULL TIME",
              font=fonts["bold_xxs"], fill="#cc5500")
    lines = wrap_title(job["title"], 18)
    y = 180
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#cc5500")
    ...
    draw.text((560, y+158), truncate(job.get("experience","Open"),14),
              font=fonts["bold_sm"], fill="#1a0a00")
    draw.text((WIDTH-72, HEIGHT-72), "Bio link ->",
              font=fonts["bold_xs"], fill="#cc5500", anchor="ra")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_newspaper(job, fonts, output):
    ...
    draw.text((WIDTH//2, 140), "DELHI NCR EMPLOYMENT GAZETTE",
              font=fonts["bold_xxs"], fill="#111111", anchor="mm")
    draw.text((WIDTH//2, 185), today_str().upper() + "  -  HIRING EDITION",
              font=fonts["mono_xs"], fill="#888888", anchor="mm")
    ...
    draw.text((72, y+100), truncate(job["company"],22),
              font=fonts["bold_sm"],  fill="#111111")
    ...
    draw.text((72, y+220), truncate(job.get("experience","Open"),18),
              font=fonts["bold_sm"], fill="#111111")
    ...
    draw.text((WIDTH//2, HEIGHT-60), "@delhi_gurugram_jobs",
              font=fonts["mono_xs"], fill="#aaaaaa", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_forest_green(job, fonts, output):
    ...
    draw.text((72, 240), "REMOTE-FRIENDLY  |  IT & TECH",
              font=fonts["bold_xxs"], fill="#4ade80")
    lines = wrap_title(job["title"], 16)
    y = 310
    for line in lines:
        draw.text((72, y), line, font=fonts["black_xl"], fill="#ffffff")
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#86efac")
    draw.text((72, y+80), truncate(job.get("location","Delhi NCR"), 28),
              font=fonts["reg_sm"], fill="#4ade8066")
    ...
    draw.text((580, y+220), truncate(job.get("experience","Open"),14),
              font=fonts["bold_sm"], fill="#ffffff")
    draw_rounded_rect(draw, [72, y+300, WIDTH-72, y+400], 14, fill="#4ade80")
    draw.text((WIDTH//2, y+350), "APPLY  -  LINK IN BIO",
              font=fonts["bold_sm"], fill="#0d2818", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_carbon_dark(job, fonts, output):
    ...
    draw = ImageDraw.Draw(img)
    draw_rounded_rect(draw, [WIDTH-190, 60, WIDTH-60, 190], 50,
                      outline="#3f3f46", width=3)
    initials = "".join(w[0].upper() for w in job["company"].split()[:2])
    draw.text((WIDTH-125, 125), initials, font=fonts["bold_sm"],
              fill="#71717a", anchor="mm")
    ...
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#a1a1aa")
    ...
    draw.text((760, y+200), truncate(job.get("experience","Open"),12),
              font=fonts["bold_sm"], fill="#fafafa", anchor="mm")
    draw_rounded_rect(draw, [72, y+300, WIDTH-72, y+420], 12, fill="#3f3f46")
    draw.text((WIDTH//2, y+360), "APPLY  -  LINK IN BIO",
              font=fonts["bold_xs"], fill="#fafafa", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_lavender(job, fonts, output):
    ...
    draw.text((226, 97), "HR & TALENT", font=fonts["bold_xs"],
              fill="#5b21b6", anchor="mm")
    draw.text((WIDTH-72, 97), today_str(), font=fonts["reg_xs"],
              fill="#a78bfa", anchor="ra")
    lines = wrap_title(job["title"], 18)
    y = 200
    for line in lines:
        ...
        y += 140
    draw.text((72, y+10), truncate(job["company"], 28),
              font=fonts["bold_sm"], fill="#7c3aed")
    draw.text((72, y+80), truncate(job.get("location","Delhi NCR"), 28),
              font=fonts["reg_sm"], fill="#a78bfa")
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((620, y+210), truncate(job.get("experience","Open"),14),
              font=fonts["bold_sm"], fill="#1e0050")
    draw_rounded_rect(draw, [72, y+330, WIDTH-72, y+430], 14, fill="#7c3aed")
    draw.text((WIDTH//2, y+380), "APPLY  -  LINK IN BIO",
              font=fonts["bold_sm"], fill="#ffffff", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def style_retro_stamp(job, fonts, output):
    ...
    draw = ImageDraw.Draw(img)
    # stamp border (dashed effect via segments)
    seg = 40
    ...
    draw.text((WIDTH//2, 160), "* JOB OPENING *",
              font=fonts["bold_xs"], fill="#2c1f0e", anchor="mm")
    draw.text((WIDTH//2, 230), truncate(job["company"], 26),
              font=fonts["serif_sm"], fill="#8b6914", anchor="mm")
    lines = wrap_title(job["title"], 18)
    y = 330
    for line in lines:
        cx = centered_x(draw, line, fonts["serif_xl"], WIDTH)
        ...
        y += 150
    draw.line([(WIDTH//2-200, y+30), (WIDTH//2+200, y+30)],
              fill="#2c1f0e44", width=3)
    sal, stype = smart_salary(job.get("salary"), job.get("title",""),
                               job.get("company",""), job.get("experience",""))
    slabel = salary_label(stype)
    ...
    draw.text((700, y+115), truncate(job.get("location","Gurugram"),14),
              font=fonts["bold_sm"], fill="#2c1f0e", anchor="mm")
    draw.text((WIDTH//2, y+200), "APPLY  |  LINK IN BIO",
              font=fonts["bold_xxs"], fill="#8b6914", anchor="mm")
    draw.text((WIDTH//2, HEIGHT-100), "@delhi_gurugram_jobs",
              font=fonts["mono_xs"], fill="#b49060", anchor="mm")
    img.save(output, "JPEG", quality=95, dpi=(72,72))
    return output
...
def pick_style(slot):
    """Pick a different style every post. Uses slot + day-of-week so
    it never repeats the same style two days in a row."""
    ist  = pytz.timezone("Asia/Kolkata")
    day  = datetime.now(ist).weekday()   # 0-6
...
def generate_card(job, slot=0):
    """Generate a 1080x1080 job card image. Returns the output file path."""
    fonts  = load_fonts()
    ...
    try:
        ...
        print(f"Style {style_fn.__name__} failed: {e} — falling back to warm cream")
        return style_warm_cream(job, fonts, output)
