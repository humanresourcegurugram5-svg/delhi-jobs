...
from clean_data      import prepare_job
...
def commit_data():
    try:
        cmds = [
            ...
            ["git", "add", "data/"],
            ["git", "commit", "-m",
             f"Data update {datetime.now(IST).strftime('%d %b %Y %H:%M IST')}"],
            ["git", "push"],
        ]
        for cmd in cmds:
        ...
        print("Data committed to GitHub")
...
def run():
    ...
    print("\nCleaning data...")
    cleaned_jobs = [prepare_job(j) for j in raw_jobs]

    print("\nFiltering duplicates...")
    unique_jobs = filter_unique_jobs(cleaned_jobs, needed=1)

    if not unique_jobs:
        print("All jobs are duplicates — using oldest from bank")
        unique_jobs = cleaned_jobs[:1]
